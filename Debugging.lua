...
--]]
--[[
This module of MountRoulette implements a simple debugging facility, enabling the
...
-- Colour to use when displaying the addon's name or msg header
local addonNameColour = "00ff00"    
local addonNameColourized = string.format( "|cff%s%s|r", addonNameColour, addonName )
...
MR.Debugging_mt = {}
MR.Debugging_mt.__index = MR.Debugging_mt
...
local function logMsg( level, format, ... )
    ...
    if( formatType == "string" ) then
        if( select( "#", ... ) > 0 ) then
            ...
            if( status ) then
                DEFAULT_CHAT_FRAME:AddMessage( string.format( "%s: %s", addonNameColourized, msg ) )
                return
            else
                DEFAULT_CHAT_FRAME:AddMessage( addonNameColourized .. ": Error formatting debug msg: " .. msg )
                return
            end
        else
            DEFAULT_CHAT_FRAME:AddMessage( addonNameColourized .. ": " .. format )
            return
        end
    ...
    DEFAULT_CHAT_FRAME:AddMessage( addonNameColourized .. ": " .. tostring( format) )
    return
end
...
function MR.Debugging_mt:new( )
    local debugObject = {}                          -- new object
    setmetatable( debugObject, MR.Debugging_mt )    -- make MR.Debugging_mt handle lookup
...
function MR.Debugging_mt:setLevel( level )
...
function MR.Debugging_mt:log( level, ... )
    local order = LEVELLIST[level]
    if( order == nil ) then
